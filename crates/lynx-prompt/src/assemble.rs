...
pub(crate) fn resolve_seg_bg(seg: &RenderedSegment, theme: &lynx_theme::schema::Theme) -> Option<String> {
    seg.cache_key
        .as_deref()
...
fn apply_width_constraints(text: &str, seg: &RenderedSegment, theme: &lynx_theme::schema::Theme) -> String {
...
fn resolve_seg_separators(seg: &RenderedSegment, theme: &lynx_theme::schema::Theme) -> SegmentSeparators {
    seg.cache_key
        .as_deref()
        .and_then(|name| theme.segment.get(name))
        .and_then(|sc| {
...
pub(crate) fn assemble(segs: &[RenderedSegment], theme: &lynx_theme::schema::Theme, sep: &Separators, is_left: bool, ctx: Option<&crate::segment::RenderContext>) -> String {
    if segs.is_empty() {
