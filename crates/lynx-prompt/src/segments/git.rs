        ...
        let text = match cfg.format.as_deref() {
            Some(tmpl) => {
                let staged_icon = cfg
                    ...
                    .and_then(|s| s.icon.as_deref())
                    .unwrap_or("+");
                let modified_icon = cfg
                    .modified
                    .as_ref()
                    .and_then(|s| s.icon.as_deref())
                    .unwrap_or("!");
                let untracked_icon = cfg
                    .untracked
                    .as_ref()
                    .and_then(|s| s.icon.as_deref())
                    .unwrap_or("?");
                let staged_val = if staged { staged_icon } else { "" };
                let modified_val = if modified { modified_icon } else { "" };
                let untracked_val = if untracked { untracked_icon } else { "" };
                apply_format(
                    tmpl,
                    &[
                        ("staged", staged_val),
...
mod tests {
    ...
    fn branch_shows_when_in_repo() {
        let r =
            GitBranchSegment.render(&empty_config(), &ctx_with_git("main", false, false, false));
        assert!(r.is_some());
    ...
    fn status_hides_when_clean() {
        let r =
            GitStatusSegment.render(&empty_config(), &ctx_with_git("main", false, false, false));
        assert!(r.is_none());
    }
    ...
    fn branch_custom_format_changes_layout() {
        ...
        let r = GitBranchSegment
            .render(&cfg, &ctx_with_git("main", false, false, false))
            .unwrap();
    ...
    fn branch_default_format_matches_current_output() {
        let r_default = GitBranchSegment
            .render(&empty_config(), &ctx_with_git("main", false, false, false))
            .unwrap();
        let cfg: toml::Value = toml::from_str(r#"format = "$icon$branch""#).unwrap();
        let r_format = GitBranchSegment
            .render(&cfg, &ctx_with_git("main", false, false, false))
            .unwrap();
    ...
    fn status_custom_format_with_brackets() {
        let cfg: toml::Value =
            toml::from_str(r#"format = "[$staged$modified$untracked]""#).unwrap();
        let r = GitStatusSegment
            .render(&cfg, &ctx_with_git("main", true, false, true))
            .unwrap();
        assert_eq!(r.text, "[+?]");
    }
    ...
    fn status_unknown_format_var_is_empty() {
        ...
        let r = GitStatusSegment
            .render(&cfg, &ctx_with_git("main", true, false, false))
            .unwrap();
        assert_eq!(r.text, "+");
    }
}
