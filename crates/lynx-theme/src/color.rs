...
pub fn named_to_rgb(name: &str) -> Option<(u8, u8, u8)> {
    match name {
        ...
        "light-grey" | "light-gray" | "bright-grey" | "bright-gray" => Some((115, 122, 162)), // #737aa2
...
mod tests {
    ...
    fn named_colors_work_on_all_capabilities() {
        for name in &[
            ...
            for cap in [
                ...
                assert!(
                    !s.is_empty(),
                    "named color '{name}' should produce output in {cap:?}"
                );
            }
        }
    }
    ...
    fn light_blue_differs_from_blue_on_truecolor() {
        ...
        assert_ne!(
            blue, light_blue,
            "light-blue should have different RGB than blue"
        );
    }
    ...
    fn hex_bg_downgrades_to_basic16() {
        ...
        assert!(
            s.starts_with("\x1b[4"),
            "expected bg basic16 code, got: {s}"
        );
    }
